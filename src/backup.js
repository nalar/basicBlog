...
app.get('/singlepost/:postid', function(request, response) {
    ...
    // Get the post associated to the given postID
    Post.findById(postID).then(function(row) {
        var blogPost = row;
        // Pass the post and its comments to the renderer
    ...
    // Get the comments related to that postID
    // Comment.findAll(
    //  { where: { id: postID } }
    // ).then(function(comments) {
    //     var data = comments.map(function(comment) {
    //         return {
    //             commentBody: comment.dataValues.body,
    //             commentAuthor: comment.dataValues.author
    //         }
    //     });
...
app.post('/addpost', function(request, response) {
    ...
    postAuthor = request.body.userID;
    ...
    Post.create({
        ...
        body: postBody,
        author: postAuthor
    }).then(function(outputfromcreate) {
        console.log(outputfromcreate);
    });
    ...
    response.redirect('/singlepost/' + postID);
});
